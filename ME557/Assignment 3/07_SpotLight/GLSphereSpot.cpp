...
void GLSphereSpot::initShader(void)
{
    ...
    _program = LoadAndCreateShaderProgram("../data/shaders/multilight.vs", "../data/shaders/multilight.fs")
#else
	// This loads the shader program from a file
    _program = LoadAndCreateShaderProgram("../../data/shaders/multilight.vs", "../../data/shaders/multilight.fs");
#endif
    ...
    _light_source1._lightPos = glm::vec4(25.0,25.0,0.0, 1.0);
    _light_source1._ambient_intensity = 0.3;
    _light_source1._specular_intensity = 10.1;
    ...
    _light_source1._ambientIdx = glGetUniformLocation(_program, "ambient_intensity1");
    _light_source1._diffuseIdx = glGetUniformLocation(_program, "diffuse_intensity1");
    _light_source1._specularIdx = glGetUniformLocation(_program, "specular_intensity1");
    _light_source1._attenuation_coeffIdx = glGetUniformLocation(_program, "attenuationCoefficient1");
    _light_source1._lightPosIdx = glGetUniformLocation(_program, "light_position1");
    _light_source1._cone_angleIdx = glGetUniformLocation(_program, "cone_angle1");
    _light_source1._cone_directionIdx = glGetUniformLocation(_program, "cone_direction1");
    ...
    //glUseProgram(0);
    //testing lightsource2
    
    // define the position of the light and send the light position to your shader program
    _light_source0._lightPos = glm::vec4(15.0,15.0,10.0, 1.0);
    _light_source0._ambient_intensity = 0.3;
    _light_source0._specular_intensity = 1.0;
    _light_source0._diffuse_intensity = 8.0;
    _light_source0._attenuation_coeff = 0.02;
    
    _light_source0._cone_angle = 20.0; // in degree
    _light_source0._cone_direction = glm::vec3(0.0, 0.0, 0.0); // this must be aligned with the object and light position.
    ...
    _light_source0._cone_angleIdx = glGetUniformLocation(_program, "cone_angle");
    _light_source0._cone_directionIdx = glGetUniformLocation(_program, "cone_direction");
    ...
    glUniform1f(_light_source0._cone_angleIdx, _light_source0._cone_angle);
    glUniform3fv(_light_source0._cone_directionIdx, 1, &_light_source0._cone_direction[0]);
